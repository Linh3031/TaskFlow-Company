<script>
    ...
    $: combinedStaff = (() => {
        ...
        allStaff.forEach(s => staffMap.set(s.id, { ...s, isGhost: false }));
        currentItemAssignees.forEach(a => {
            if (!staffMap.has(a.id)) {
                staffMap.set(a.id, { id: a.id, username: a.username, role: 'unknown', isGhost: true });
            }
        });
        return Array.from(staffMap.values());
    })();

    $: filteredStaff = combinedStaff
                ...
                <div class="max-h-56 overflow-y-auto bg-slate-50 border border-slate-200 rounded-lg p-1.5 space-y-0.5">
                    {#each filteredStaff as s}
                        <label class="flex items-center gap-3 p-2 hover:bg-cyan-100 rounded-md cursor-pointer transition-colors border border-transparent hover:border-cyan-200 {selectedStaffIds.includes(s.id) ? 'bg-cyan-50 border-cyan-200' : ''}">
                            ...
                            <span class="text-sm font-semibold {s.isGhost ? 'text-red-500 line-through' : 'text-slate-700'}">
                                {s.username} {#if s.isGhost}<span class="text-[9px] bg-red-100 text-red-600 px-1 py-0.5 rounded ml-1 not-italic border border-red-200">Đã xóa</span>{/if}
                            </span>
                        </label>
                    {/each}
