<script>
    ...
    import { collection, doc, getDoc, setDoc, onSnapshot, query, where, updateDoc, serverTimestamp, addDoc, runTransaction } from 'firebase/firestore';
    ...
    async function writeDebugLog(action, reason) { try { await addDoc(collection(db, 'debug_8nttt_logs'), { action, storeId: activeStoreId, dateStr, reason, user: $currentUser?.username || 'unknown', timestamp: serverTimestamp() }); } catch (e) { } }
    function getOldFormatDate(dStr) { if (!dStr) return ''; const parts = dStr.split('-'); if (parts.length !== 3) return dStr; return `${parts[0]}-${parseInt(parts[1], 10)}-${parseInt(parts[2], 10)}`; }
    ...
    // ĐÃ PHẪU THUẬT: Chỉ tốn đúng 1 Lượt Đọc để lấy toàn bộ dữ liệu 1 tháng
    async function loadAndShowStats() {
        ...
        try {
            // Document tổng hợp tự động gom vào định dạng VD: IDStore_2026-03
            ...
            if (snap.exists() && snap.data().users) {
                // Biến object thành array mà file ChecklistStatsModal đang cần
    ...
    async function openAdminModal(event) {
        ...
        if (item) { editingAreaId = item.id; newAreaName = item.areaName; selectedStaffIds = (item.assignees || []).map(a => a.id); } 
        else { editingAreaId = null; newAreaName = ''; selectedStaffIds = []; }
    }
    ...
    async function saveAreaToTemplate() {
        ...
        else { newItemData = { id: 'area_' + Date.now(), areaName: newAreaName.trim(), assignees: assigneesData }; currentItems.push(newItemData); }
    ...
    async function deleteArea(event) {
        ...
        const dailyRef = getDailyRecordRef();
        const dailySnap = await getDoc(dailyRef);
        if (dailySnap.exists()) { let dailyItems = dailySnap.data().items || []; await updateDoc(dailyRef, { items: dailyItems.filter(i => i.id !== id) }); }
    }
    ...
    // ĐÃ PHẪU THUẬT: Vừa ghi dữ liệu Ngày, vừa lưu thống kê cho Tháng
    async function handleUploadImage(eventObj) {
        ...
        try {
            ...
            const dailyRef = getDailyRecordRef();
            // Lấy ra khóa YYYY-MM (VD: 2026-03) và Ngày (VD: 13)
            ...
            await runTransaction(db, async (transaction) => {
                // Đọc bản ghi ngày và bản ghi tổng hợp
                const dailyDoc = await transaction.get(dailyRef);
                const monthlyDoc = await transaction.get(monthlyStatsRef);
                
                // 1. Cập nhật dữ liệu cho Ngày
                ...
                // 2. Cập nhật dữ liệu vào Bản Ghi Tổng Hợp Tháng
