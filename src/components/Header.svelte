<script>
  ...
  onMount(() => {
    ...
    // 3. Admin luôn thấy để test (Tùy chọn, có thể bỏ nếu muốn)
...
<header class="app-header bg-white shadow-sm px-4 py-3 flex justify-between items-center z-10 shrink-0 sticky top-0">
  ...
  <div class="flex items-center gap-2">
    ...
    <button class="w-8 h-8 flex items-center justify-center text-purple-500 bg-purple-50 hover:bg-purple-100 rounded-full transition-colors" on:click={() => dispatch('openTour')} title="Xem hướng dẫn">
      <span class="material-icons-round text-xl">help_outline</span>
    </button>

    {#if $currentUser?.role === 'admin' || $currentUser?.role === 'super_admin'}
      <button class="w-8 h-8 flex items-center justify-center text-orange-500 bg-orange-50 hover:bg-orange-100 rounded-full transition-colors" on:click={() => dispatch('openAdmin')} title="Cài đặt">
        <span class="material-icons-round text-xl">settings</span>
      </button>
    {/if}
    
    <button class="w-8 h-8 flex items-center justify-center text-gray-400 hover:text-red-500 hover:bg-red-50 rounded-full transition-colors" on:click={handleLogout} title="Đăng xuất">
