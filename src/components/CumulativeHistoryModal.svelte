        ...
        for (let i = 0; i < 3; i++) {
            ...
            try {
                ...
                if (snap.exists()) {
                    ...
                    // [NEW FIX] Ưu tiên lấy baselineStats (Lịch đã Chốt Gốc), sau đó mới lấy systemStats, cuối cùng là stats
                    historyData.push({ month: `${m}/${y}`, data: data.baselineStats || data.systemStats || data.stats || [] });
                } else {
    ...
    function processAggregatedData() {
        ...
        historyData.forEach((hObj, monthIndex) => {
            hObj.data.forEach(staff => {
                ...
                // [NEW] Bổ sung tính Tổng NV (nv)
                map[staff.id].history[monthIndex] = {
                    gh: staff.gh || 0,
                    tn: staff.tn || 0,
                    kho: staff.kho || 0,
                    nv: (Number(staff.gh)||0) + (Number(staff.tn)||0) + (Number(staff.kho)||0),
        ...
        aggregatedStats = Object.values(map).map(staff => {
            ...
            const past1 = staff.history[1] || { gh: 0, tn: 0, kho: 0, nv: 0, weekend: 0 };
            const past2 = staff.history[2] || { gh: 0, tn: 0, kho: 0, nv: 0, weekend: 0 };
            
            const currNV = (Number(curr.gh)||0) + (Number(curr.tn)||0) + (Number(curr.kho)||0);
...
<div class="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[100] flex items-center justify-center p-4">
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-[1400px] flex flex-col max-h-[90vh] overflow-hidden animate-popIn">
        ...
        <div class="flex items-center justify-between p-4 border-b bg-slate-50">
            <div class="flex items-center gap-3">
                ...
                <div>
                    ...
                    <p class="text-xs text-slate-500">Đối chiếu Lịch đã Chốt Gốc để cân bằng (Lịch sửa tay lẻ tẻ sẽ không bị tính)</p>
                </div>
            </div>
        ...
        <div class="p-4 overflow-auto flex-1 bg-slate-50/50">
            ...
            {:else}
                <table class="w-full text-sm bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden text-left border-collapse min-w-max">
                    <thead>
                        <tr class="bg-slate-800 text-slate-200 text-xs">
                            <th class="p-3 font-bold border-r border-slate-700 w-48">Nhân viên</th>
                            
                            <th class="p-3 text-center border-r border-slate-700 bg-slate-700/80 cursor-pointer hover:bg-slate-600 transition-colors w-14" on:click={() => toggleSort('totalGH')}>
                            ...
                            <th class="p-3 text-center border-r border-slate-700 bg-slate-700/80 cursor-pointer hover:bg-slate-600 transition-colors w-14" on:click={() => toggleSort('totalTN')}>
                            ...
                            <th class="p-3 text-center border-r border-slate-700 bg-slate-700/80 cursor-pointer hover:bg-slate-600 transition-colors w-14" on:click={() => toggleSort('totalKho')}>
                            ...
                            <th class="p-3 text-center border-r border-slate-700 bg-slate-700/80 cursor-pointer hover:bg-slate-600 transition-colors w-14" on:click={() => toggleSort('totalNV')}>
                                <div class="font-bold text-emerald-300">Tổng NV</div>
                                <div class="text-[10px] font-normal text-slate-400 mt-0.5">{sortField === 'totalNV' ? (sortDirection === 'desc' ? '▼' : '▲') : '↕'}</div>
                            </th>
                            <th class="p-3 text-center border-r border-slate-700 bg-slate-700/80 cursor-pointer hover:bg-slate-600 transition-colors w-14" on:click={() => toggleSort('totalWeekend')}>
                            ...
                            <th class="p-3 text-center border-r border-slate-700 bg-indigo-900/50">
                                ...
                                <div class="text-[10px] font-normal text-slate-400 mt-0.5">GH · TN · K · Tổng NV · Cuối tuần</div>
                            </th>
                    ...
                    <tbody class="divide-y divide-slate-100">
                        {#each sortedData as staff, i}
                            ...
                            <tr class="hover:bg-slate-50 transition-colors {sortField !== 'default' ? 'bg-slate-50/50' : ''}">
                                ...
                                <td class="p-3 border-r border-slate-100 bg-emerald-50 text-center">
                                    <span class="inline-block w-6 text-center {staff.totalNV>0?'text-emerald-700 font-bold bg-emerald-100 rounded':'text-slate-300'}">{staff.totalNV||'-'}</span>
                                </td>
                                <td class="p-3 border-r border-slate-100 bg-amber-50/30 text-center">
                                ...
                                {#each staff.history.slice(1) as d}
                                    <td class="p-3 border-r border-slate-100">
                                        ...
                                        {:else}
                                            <div class="flex justify-center gap-1.5 text-xs opacity-80">
                                                ...
                                                <span class="w-5 text-center {d.nv>0?'text-emerald-600 font-bold':'text-slate-300'}">{d.nv||'-'}</span>
