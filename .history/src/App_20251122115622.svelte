// Version 3.0 - Multi-Store Logic Integration
<script>
  ...
  onMount(() => {
    // Chỉ chạy khi đã có user đăng nhập
    if (!$currentUser) return;

    const myStoreId = $currentUser.storeId; // Lấy mã kho của user
    if (!myStoreId) { alert("Lỗi: Tài khoản không có Mã Kho!"); return; }

    // 1. Đồng bộ Template RIÊNG của kho này
    // Document ID sẽ là: template_908, template_KHO1,...
    ...
    const unsubTemplate = onSnapshot(doc(db, 'settings', templateDocId), (docSnap) => {
      if (docSnap.exists()) {
        ...
        // Nếu là Admin/Super Admin của kho này mà chưa có mẫu -> Tạo mẫu mặc định riêng cho kho
    ...
    // 2. Đồng bộ Công việc Hôm Nay (Chỉ lấy việc của Kho này)
    const today = getTodayStr();
    const q = query(
        collection(db, 'tasks'), 
        where('date', '==', today),
        where('storeId', '==', myStoreId) // QUAN TRỌNG: Lọc theo kho
    );
    ...
    const unsubTasks = onSnapshot(q, (snapshot) => {
      ...
      // Tự động tạo công việc nếu danh sách rỗng (Chỉ Admin/Super Admin kích hoạt)
  ...
  // Hàm khởi tạo công việc đầu ngày (Có gắn mã kho)
      ...
      if (template[type]) {
        template[type].forEach(item => {
          ...
          batch.set(newRef, {
            ...
            createdBy: 'System', 
            date: today, 
            storeId: storeId, // GẮN MÃ KHO VÀO TASK
  ...
  // ... (Giữ nguyên các hàm handleTaskClick, confirmComplete)
...
<main>
  ...
  {:else}
    <div class="app-container">
      ...
      <footer>Design by 3031 | Kho: {$currentUser.storeId}</footer>
    </div>
  {/if}
