// Version 3.0 - Admin Multi-Store Logic
<script>
  ...
  // Mã kho hiện tại của Admin đang thao tác
  const myStoreId = $currentUser?.storeId; 

  // --- LOGIC UPLOAD EXCEL ĐA KHO ---
    ...
    try {
      ...
      rawData.forEach(row => {
        ...
        if (uName) {
          ...
          // LOGIC MÃ KHO:
          // 1. Ưu tiên cột 'makho' trong Excel (cho Super Admin upload nhiều kho)
          // 2. Nếu không có, lấy mã kho của chính người upload (Admin kho tạo user cho kho mình)
          let targetStore = safeString(nRow.makho || nRow.storecode || nRow.storeid);
          ...
          batch.set(userRef, {
            ...
            storeId: targetStore // Lưu mã kho
          }, { merge: true }); // Merge để không mất dữ liệu cũ nếu có
          count++;
        }
      });
      await batch.commit();
      alert(`✅ Đã đồng bộ ${count} nhân viên (Kho: ${myStoreId})!`);
    } catch (err) {
  ...
  // --- LOGIC TEMPLATE ĐA KHO ---
  async function addTemplateTask() {
    if (!newTaskTitle.trim()) return;

    // 1. Lưu Mẫu (Vào đúng document của Kho này)
    taskTemplate.update(current => {
      ...
      // Lưu vào settings/template_908 (ví dụ)
      setDoc(doc(db, 'settings', `template_${myStoreId}`), updated);
      return updated;
    });

    // 2. Thêm NGAY vào danh sách hôm nay (Của Kho này)
    try {
      await addDoc(collection(db, 'tasks'), {
        ...
        createdBy: 'Admin Add',
        date: getTodayStr(),
        storeId: myStoreId, // Quan trọng: Gắn mã kho
  ...
  function removeTemplateTask(index) {
    if (!confirm('LƯU Ý: Chỉ xóa trong MẪU. Việc hôm nay vẫn còn.\nXóa chứ?')) return;
    
    taskTemplate.update(current => {
      const updated = { ...current };
      updated[activeType].splice(index, 1);
      // Lưu vào settings/template_{storeId}
      setDoc(doc(db, 'settings', `template_${myStoreId}`), updated);
      return updated;
    });
  }
...
<div class="fixed inset-0 bg-black/50 z-50 flex items-center justify-center backdrop-blur-sm p-4">
  <div class="bg-white w-full max-w-md rounded-2xl shadow-2xl flex flex-col max-h-[90vh] overflow-hidden animate-popIn">
    ...
    <div class="p-4 border-b flex items-center gap-2 bg-slate-50">
      ...
      <h3 class="text-lg font-bold text-slate-800">Admin Kho: {myStoreId}</h3>
    </div>
    ...
    <div class="p-4 overflow-y-auto flex-1">
      <div class="mb-6">
        ...
        <label for="excel-upload" class="flex items-center justify-center gap-2 w-full p-3 bg-slate-100 border border-dashed border-slate-300 rounded-lg cursor-pointer hover:bg-slate-200 transition-colors text-slate-600 font-semibold">
          ...
          {isUploading ? 'Đang xử lý...' : 'Nạp Excel (Cột: user, pass, makho)'}
        </label>
        <input id="excel-upload" type="file" hidden accept=".xlsx, .xls" on:change={handleExcelUpload} disabled={isUploading} />
        <p class="text-xs text-gray-400 mt-1 italic">*File Excel cần cột "makho" nếu muốn tạo cho kho khác.</p>
      </div>
      ...
      <div>
        ...
        <div class="flex gap-2 mb-3">
          <input type="text" bind:value={newTime} class="w-20 text-center p-2 border border-slate-200 rounded-lg focus:border-orange-500 outline-none" placeholder="08:00">
          <input type="text" bind:value={newTaskTitle} class="flex-1 p-2 border border-slate-200 rounded-lg focus:border-orange-500 outline-none" placeholder="Tên việc..." on:keydown={(e) => e.key === 'Enter' && addTemplateTask()}>
          <button class="w-10 h-10 flex items-center justify-center bg-orange-500 text-white rounded-lg hover:bg-orange-600 active:scale-95 transition-all shadow-md disabled:opacity-50" on:click={addTemplateTask} disabled={!newTaskTitle.trim()}>
          ...
          {#if $taskTemplate[activeType]}
            {#each $taskTemplate[activeType] as item, i}
              <li class="flex justify-between items-center p-3 bg-white hover:bg-slate-50 transition-colors">
                ...
                <button class="text-red-400 hover:text-red-600 p-1 rounded-full hover:bg-red-50 transition-colors" on:click={() => removeTemplateTask(i)}>
    ...
    <div class="p-4 border-t bg-slate-50">
      <button class="w-full py-3 rounded-xl font-bold text-slate-700 bg-slate-200 hover:bg-slate-300 active:bg-slate-400 transition-colors" on:click={() => dispatch('close')}>Đóng</button>
    </div>
  </div>
</div>
