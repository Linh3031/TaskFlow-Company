<script>
  ...
  // --- SỬA LỖI QUAN TRỌNG: Dùng $: để biến tự động cập nhật khi user thay đổi ---
  ...
  async function createStore() {
    ...
    try {
        await setDoc(doc(db, 'stores', cleanId), { name: newStoreName.trim(), createdAt: serverTimestamp() });
        alert(`✅ Đã tạo kho: ${newStoreName} (${cleanId})`);
        newStoreId = ''; newStoreName = '';
    } catch(e) { alert("Lỗi: " + e.message); }
  }

  // --- [SUPER ADMIN] 2. TẠO ADMIN KHO ---
  async function createAdminAccount() {
    if(!newAdminUser || !newAdminPass || !targetStoreForAdmin) return alert("Nhập thiếu thông tin!");
    try {
        const cleanUser = newAdminUser.trim().toLowerCase();
        await setDoc(doc(db, 'users', cleanUser), {
            username: cleanUser, username_idx: cleanUser, pass: newAdminPass.trim(),
            name: `Admin Kho ${targetStoreForAdmin}`, role: 'admin', storeId: targetStoreForAdmin, createdAt: serverTimestamp()
        });
        alert(`✅ Đã tạo Admin: ${cleanUser} cho kho ${targetStoreForAdmin}`);
      ...
      rawData.forEach(row => {
        ...
        if (uName) {
          batch.set(doc(db, 'users', uName.toLowerCase()), {
            ...
            name: nRow.name ? safeString(nRow.name) : uName, role: 'staff', storeId: myStoreId
          }, { merge: true });
          count++;
        }
      });
      await batch.commit(); alert(`✅ Xong! ${count} nhân viên.`);
    } catch (err) { alert("Lỗi: " + err.message); } finally { isUploading = false; event.target.value = ''; }
  }
  ...
  async function addTemplateTask() {
    ...
    try { await addDoc(collection(db, 'tasks'), { type: activeType, title: newTaskTitle, timeSlot: newTime, completed: false, createdBy: 'Admin', date: getTodayStr(), storeId: myStoreId, timestamp: serverTimestamp() }); } catch(e){}
    newTaskTitle = '';
  }
  ...
  <div class="bg-white w-full max-w-md rounded-2xl shadow-2xl flex flex-col max-h-[90vh] overflow-hidden animate-popIn">
    ...
    <div class="p-4 border-b flex items-center gap-2 bg-slate-50">
      ...
      <div class="flex-1">
          ...
          {#if isSuperAdmin}<p class="text-xs text-purple-600 font-bold">Quản trị hệ thống toàn cục</p>{/if}
      </div>
    </div>
    ...
    <div class="p-4 overflow-y-auto flex-1">
        {#if isSuperAdmin}
            <div class="flex gap-2 mb-4 border-b pb-2">
                <button class="px-3 py-1 rounded-lg text-sm font-bold transition-colors {saTab==='store'?'bg-purple-600 text-white':'bg-gray-100 text-gray-600'}" on:click={()=>saTab='store'}>1. Quản lý Kho</button>
                <button class="px-3 py-1 rounded-lg text-sm font-bold transition-colors {saTab==='account'?'bg-purple-600 text-white':'bg-gray-100 text-gray-600'}" on:click={()=>saTab='account'}>2. Tạo Admin Kho</button>
            </div>

            {#if saTab === 'store'}
                <div class="space-y-3">
                    <div class="bg-blue-50 p-3 rounded border border-blue-100">
                        <h4 class="text-xs font-bold text-blue-600 uppercase mb-2">Thêm Kho Mới</h4>
                        <input type="text" bind:value={newStoreId} class="w-full p-2 border rounded uppercase mb-2 bg-white" placeholder="Mã Kho (VD: 908)">
                        <input type="text" bind:value={newStoreName} class="w-full p-2 border rounded bg-white mb-2" placeholder="Tên hiển thị">
                        <button class="w-full py-2 bg-blue-600 text-white rounded font-bold hover:bg-blue-700" on:click={createStore}>Tạo Kho</button>
                    </div>
                    ...
                    <div class="mt-4">
                        ...
                        <ul class="bg-gray-50 rounded border divide-y max-h-40 overflow-y-auto">
                            {#each $storeList as store}
                                <li class="p-2 text-sm flex justify-between items-center"><span>{store.name}</span> <span class="font-mono font-bold bg-gray-200 px-2 py-1 rounded text-xs">{store.id}</span></li>
                            {/each}
                        </ul>
                    </div>
                </div>
            {:else}
                <div class="space-y-3">
                    <h4 class="text-xs font-bold text-gray-500 uppercase">Cấp quyền Admin</h4>
                    <input type="text" bind:value={newAdminUser} class="w-full p-2 border rounded" placeholder="Username Admin">
                    ...
                    <button class="w-full py-2 bg-purple-600 text-white rounded font-bold hover:bg-purple-700" on:click={createAdminAccount}>Cấp Quyền Admin</button>
                </div>
            {/if}
        ...
        {:else}
            <div class="mb-6">
                ...
                <label class="flex items-center justify-center gap-2 w-full p-3 bg-slate-100 border border-dashed border-slate-300 rounded-lg cursor-pointer hover:bg-slate-200 font-semibold text-slate-600">
                    <span class="material-icons-round">cloud_upload</span> {isUploading ? '...' : 'Nạp Excel'}
                </label>
            ...
            <div>
                ...
                <ul class="border rounded-lg divide-y max-h-40 overflow-y-auto bg-slate-50">
                    {#if $taskTemplate[activeType]}
                        ...
                        <li class="flex justify-between p-2 text-sm items-center"><span><b class="bg-slate-200 px-1 rounded text-xs mr-1">{item.time}</b> {item.title}</span> <button class="text-red-500" on:click={()=>removeTemplateTask(i)}>x</button></li>
                        {/each}
                    {/if}
                </ul>
            </div>
