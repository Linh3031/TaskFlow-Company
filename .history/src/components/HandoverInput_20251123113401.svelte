...
<div class="bg-white p-3 rounded-xl shadow-sm border border-gray-100 flex flex-wrap gap-2 mb-4 items-center animate-popIn">
  {#if myStores.length > 1}
    <select bind:value={selectedTargetStore} class="p-2 border rounded-lg text-xs font-bold bg-indigo-50 text-indigo-700 outline-none">
  ...
  <div class="relative">
      ...
      <input type="datetime-local" bind:value={deadline} class="pl-7 pr-2 py-2 border rounded-lg text-sm bg-gray-50 outline-none">
  </div>
  <input type="text" bind:value={taskTitle} placeholder="Nhập việc bàn giao..." class="flex-1 p-2 border rounded-lg text-sm outline-none" on:keydown={(e) => e.key === 'Enter' && addTask()}>
  <button class="w-10 h-10 bg-purple-600 text-white rounded-lg flex items-center justify-center shadow-md" on:click={addTask}><span class="material-icons-round">send</span></button>
</div>
