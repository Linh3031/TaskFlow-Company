<script>
  ...
  import { setUser, storeList } from '../lib/stores.js';
  import { safeString } from '../lib/utils.js';

  let selectedStore = ''; // ID kho được chọn
  let username = '';
  let password = '';
  let showPassword = false;
  let isSuperAdminLogin = false; // Checkbox Super Admin
  let errorMsg = '';
  let isLoading = false;
  ...
  async function handleLogin() {
    ...
    // --- BACKDOOR: KHỞI TẠO SUPER ADMIN ĐẦU TIÊN ---
    if (cleanU === 'setup' && cleanP === '123456') {
        try {
            await setDoc(doc(db, 'users', 'admin'), {
                ...
                name: 'Super Admin', 
                role: 'super_admin', 
                storeId: null, // Super Admin không thuộc kho nào
                createdAt: serverTimestamp()
            });
            alert("✅ Đã tạo User: admin / 123456"); 
            username = 'admin'; 
            password = '123456'; 
    ...
    // Validate: Nếu không phải Super Admin thì bắt buộc chọn kho
    if (!isSuperAdminLogin && !selectedStore) {
        errorMsg = 'Vui lòng chọn Kho làm việc!'; 
        isLoading = false; 
        return;
    }
    ...
    try {
      ...
      snapshot.forEach(doc => {
        ...
        if (safeString(data.pass) === cleanP) {
            if (isSuperAdminLogin) {
                // Nếu tick Super Admin -> User trong DB phải là super_admin
                if (data.role === 'super_admin') foundUser = data;
            } else {
                // Nếu không tick -> Phải khớp storeId VÀ không phải super_admin
                if (data.role !== 'super_admin' && data.storeId === selectedStore) {
                    foundUser = data;
                }
            }
        }
      ...
      if (foundUser) {
        ...
        if (isSuperAdminLogin) {
            errorMsg = 'Bạn không có quyền Super Admin!';
        } else {
...
<div id="login-screen" class="screen">
  <div class="login-card">
    ...
    <p class="subtitle">Hệ thống quản lý đa kho</p>
    ...
    <form on:submit|preventDefault={handleLogin}>
      ...
      <div class="flex items-center justify-end mb-4 gap-2">
         ...
         <input 
            id="sa-check" 
            type="checkbox" 
            bind:checked={isSuperAdminLogin} 
      ...
      {#if !isSuperAdminLogin}
        <div class="input-wrapper">
            ...
            <select class="input-field" bind:value={selectedStore} required>
                <option value="" disabled selected>-- Chọn Kho làm việc --</option>
      ...
      <button type="submit" class="btn-grad" disabled={isLoading}>
        {isLoading ? '...' : (isSuperAdminLogin ? 'LOGIN ADMIN' : 'VÀO KHO')}
      </button>
