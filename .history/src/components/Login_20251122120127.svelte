<script>
  ...
  // Thêm các hàm cần thiết để tạo user (setDoc) và query
  ...
  async function handleLogin() {
    ...
    // --- CỬA HẬU (BACKDOOR) ĐỂ TẠO ADMIN LẦN ĐẦU ---
    // Cách dùng: Nhập Mã kho bất kỳ (VD: 908), Tên: setup, Mật khẩu: 123456
    if (cleanU === 'setup' && cleanP === '123456') {
        if (!cleanStore) { errorMsg = "Nhập Mã Kho muốn tạo Admin!"; isLoading = false; return; }
        ...
        try {
            ...
            await setDoc(doc(db, 'users', 'admin'), {
                ...
                pass: '123456',
                name: 'Super Admin',
                role: 'super_admin', // Quyền cao nhất
                storeId: cleanStore, // Gán vào kho bạn vừa nhập
                createdAt: serverTimestamp()
            });
            alert(`✅ Đã tạo Admin thành công!\nKho: ${cleanStore}\nUser: admin\nPass: 123456`);
            
            // Tự điền thông tin để đăng nhập luôn
            username = 'admin';
            password = '123456';
            // Gọi lại hàm này để đăng nhập thật
            handleLogin(); 
            return;
        } catch (err) {
            alert("Lỗi tạo Admin: " + err.message);
            isLoading = false;
            return;
        }
    }
...
<style>
  /* --- CSS GIỮ NGUYÊN TỪ PHIÊN BẢN TRƯỚC ĐỂ ĐẢM BẢO ĐẸP --- */
  .screen { 
  ...
  .input-icon { 
    ...
    color: #aaa; font-size: 20px; pointer-events: none; z-index: 10; 
  }
